...
<img class="sticker" id="3" onclick="SetActive(this.id)" src="<?php echo URLROOT; ?>/stickers/crown_small.png"/>
<img class="sticker" id="4" onclick="SetActive(this.id)" src="<?php echo URLROOT; ?>/stickers/warp_field.png"/>


	  <div class="card card-body bg-light mt-5">
        <h2>Post cancelled</h2>
        <p>(It should no longer show up on server.)</p>
        <form action="<?php echo URLROOT; ?>/posts/add" method="post">
		  ...
		  <form action="<?php echo URLROOT; ?>/posts/cancel" method="post">
		  <div class="form-group">
			<input type="hidden" name="image" id="image" class="form-control form-control-lg <?php echo (!empty($data['image_err'])) ? 'is-invalid' : ''; ?>" value="">
            <span class="invalid-feedback"><?php echo $data['image_err']; ?></span>
          </div>
		  ...
		  <input type="submit" class="btn btn-danger" id="submit-btn" value="Cancel and delete image">
        </form>
      </div>
